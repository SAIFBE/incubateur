...
const Icons = {
  ...
  message: <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M21 12c0 4.418-4.03 8-9 8a9.77 9.77 0 01-4-.83L3 20l1.27-3.8A7.53 7.53 0 013 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>,
};
...
const Sidebar = ({ role, isOpen, onClose }) => {
  ...
  const navLinks = role === 'admin' ? [
    ...
    { path: '/dashboard/admin/accepted-projects', label: 'Projets acceptes', icon: Icons.acceptedProjects },
    { path: '/dashboard/admin/events', label: 'Evenements', icon: Icons.calendar },
    { path: '/dashboard/admin/opportunities', label: 'Opportunites', icon: Icons.star },
    { path: '/dashboard/admin/highlights', label: 'Moments d\'Impact', icon: Icons.camera },
    { path: '/dashboard/admin/contact-messages', label: 'Messages contact', icon: Icons.message },
    { path: '/dashboard/admin/staff-contacts', label: 'Personnel', icon: Icons.plus },
    { path: '/dashboard/admin/statistics', label: 'Statistiques', icon: Icons.chart },
  ] : [
    ...
    <aside className={`dashboard-sidebar ${role}-theme ${isOpen ? 'open' : ''}`}>
      ...
      <div className="sidebar-footer">
        ...
        <button className="btn btn-ghost w-full" style={{ justifyContent: 'flex-start', color: 'var(--color-text-secondary)' }} onClick={handleLogout}>
          <span className="nav-icon mr-3">{Icons.logout}</span> Deconnexion
        </button>
      </div>
    </aside>
  );
